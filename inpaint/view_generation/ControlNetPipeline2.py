...
def preprocess_mask(mask, scale_factor=8, mask_blend_kernel=-1):
    ...
    Image.fromarray((mask[0, 0] * 255).astype(np.uint8)).save(f"mask.png")
...
class ControlNetPipeline(StableDiffusionControlNetImg2ImgPipeline):
    ...
    def __call__(
        ...
        # Image.fromarray((mask[0,0].cpu().numpy()*255).astype(np.uint8)).save(f"mask.png")
        # 8. Denoising loop
