...
class Deblurring(A_functions):
    ...
    def A_pinv(self, vec):
        temp = self.Ut(vec)
        singulars = self._singulars.repeat(1, self.channels).reshape(-1)
...
class ControlNetMasaCachePipeline(StableDiffusionControlNetImg2ImgPipeline):
    ...
    def __call__(
        ...
        align_var=False,
        repeat_loop : bool = False,
        replace_latent_list: Union[torch.FloatTensor, PIL.Image.Image] = None,
    ):
        r"""
        The call function to the pipeline for generation.
        ...
        with tqdm(total=num_inference_steps, desc=desc) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if replace_latent_list is not None:
                    latents_img[:1] = replace_latent_list[-1-i]
                ...
                if latent_list is not None:
                    ...
                    #latents_img[:1, :, :, :] = latent_list[0][-i-2]
                    if t > 1000 * (1 - inpainting_strength) and mask_image is not None:
                        latents_img= (latent_list[-i-2] * mask_cat) + (
                            latents_img * (1 - mask_cat)
                        )
                        if align_var:
                            latents_img[1:2, :, :, :] = masked_adain_v(latents_img[1:2, :, :, :], latent_list[1][-i-2], (1 - mask_cat[1:2]).bool(), (1 - mask_cat[1:2]).bool())
...
class ControlNetMasaCachePipeline2(StableDiffusionControlNetImg2ImgPipeline):
    ...
    def __call__(
        ...
        replace_latent_list: Union[torch.FloatTensor, PIL.Image.Image] = None,
        ddnm : bool = False,
        ddnm_strength: Optional[float] = 0.4,
        ...
        if ddnm:
            # scale = 2
            # A = torch.nn.AdaptiveAvgPool2d((64//scale,64//scale))
            # Ap = lambda z: MeanUpsample(z,scale)
            
            A_funcs = Deblurring(torch.Tensor([1 / 9] * 9).to(self.device), 4, 64, self.device)
            A = lambda z: A_funcs.A(z)
            Ap = lambda z: A_funcs.A_pinv(z)
            y = A(latents_img_ori)
            b, hwc = y.size()
            hw = hwc / 4
            h = w = int(hw ** 0.5)
        ...
        with tqdm(total=num_inference_steps, desc=desc) as progress_bar:
            for i, t in enumerate(timesteps):
                for j in range(repeat+1): 
                    ...
                    # if ddnm and t > 1000 * (1 - ddnm_strength):
                    if ddnm:
                        
                        latents_img = self.step_ddnm(noise_pred, t, latents_img,y,A,Ap, strength)
                        # prev_timestep = t - self.scheduler.config.num_train_timesteps // self.scheduler.num_inference_steps
                        # alpha_prod_t = self.scheduler.alphas_cumprod[t]
                        # alpha_prod_t_prev = self.scheduler.alphas_cumprod[prev_timestep] if prev_timestep > 0 else self.scheduler.final_alpha_cumprod
                        # beta_prod_t = 1 - alpha_prod_t
                        # pred_x0 = (latents_img - beta_prod_t**0.5 * noise_pred) / alpha_prod_t**0.5
                        # # sigma_t = (1 - alpha_prod_t_prev**2).sqrt()
                        # scale = 4
                        # # sigma_y = 0
                        # A = torch.nn.AdaptiveAvgPool2d((64//scale,64//scale))
                        # Ap = lambda z: MeanUpsample(z,scale)
                        
                        # # if sigma_t >= alpha_prod_t_prev*sigma_y:
                        # #     lambda_t = 1.
                        # #     gamma_t = (sigma_t**2 - (alpha_prod_t_prev*sigma_y)**2).sqrt()
                        # # else:
                        # #     lambda_t = (sigma_t)/(alpha_prod_t_prev*sigma_y)
                        # #     gamma_t = 0.
                            
                        # # c1 = (1 - alpha_prod_t_prev).sqrt() * eta
                        # # c2 = (1 - alpha_prod_t_prev).sqrt() * ((1 - eta ** 2) ** 0.5)
                                
                        # # x0_t_hat = pred_x0 - lambda_t*Ap(A(pred_x0) - A(latents_img_ori))
                        # x0_t_hat = pred_x0 - Ap(A(pred_x0) - A(latents_img_ori))
                        # pred_dir = (1 - alpha_prod_t_prev)**0.5 * noise_pred
                        # latents_img = alpha_prod_t_prev**0.5 * pred_x0 + pred_dir
                        
                        # latents_img = alpha_prod_t_prev.sqrt() * x0_t_hat + gamma_t * (c1 * torch.randn_like(pred_x0) + c2 * noise_pred)
                    else:
                        latents_img = self.step(noise_pred, t, latents_img)
                    ...
                    if j < repeat:
                        latents_img = self.undo_step(t, latents_img)
                        
                    # call the callback, if provided
                    if j==repeat and (i == len(timesteps) - 1 or ((i + 1) > num_warmup_steps and (i + 1) % self.scheduler.order == 0)):
                        progress_bar.update()
                        if callback is not None and i % callback_steps == 0:
                            step_idx = i // getattr(self.scheduler, "order", 1)
                            callback(step_idx, t, latents)
    ...
    def undo_step(self, timestep, x):
        noise = torch.randn(x.shape, device=x.device)
        prev_timestep = timestep - self.scheduler.config.num_train_timesteps // self.scheduler.num_inference_steps
        alpha_prod_t = self.scheduler.alphas_cumprod[timestep]
        alpha_prod_t_prev = self.scheduler.alphas_cumprod[prev_timestep] if prev_timestep > 0 else self.scheduler.final_alpha_cumprod
        beta_prod_t = 1 - alpha_prod_t
        x_next = alpha_prod_t**0.5 / alpha_prod_t_prev**0.5 * (x - (1 - alpha_prod_t_prev)**0.5 * noise) + beta_prod_t**0.5 * noise
        
        # noise = torch.randn(x.shape, device=x.device)
        # alpha_prod_t = self.scheduler.alphas_cumprod[timestep] if timestep >= 0 else self.scheduler.final_alpha_cumprod
        # beta_prod_t = 1 - alpha_prod_t
        # x_next = (1 - beta_prod_t) ** 0.5 * x + beta_prod_t**0.5 * noise
        return x_next
    ...
    def next_step_ddnm(
        ...
        if ddnm:
            ...
            sigma_t = (1 - alpha_prod_t_next**2).sqrt()
            # if sigma_t >= alpha_prod_t_prev*sigma_y:
            #     lambda_t = 1.
            #     gamma_t = (sigma_t**2 - (alpha_prod_t_prev*sigma_y)**2).sqrt()
            # else:
            ...
            # lambda_t = 1 if timestep >= 200 else 0
            # lambda_t = 1
            lambda_t = math.sin(math.pi/2*timestep/(self.scheduler.config.num_train_timesteps*strength))
            gamma_t = (sigma_t**2 - (alpha_prod_t_next*sigma_y)**2).sqrt()
            
            c1 = (1 - alpha_prod_t_next).sqrt() * eta
            c2 = (1 - alpha_prod_t_next).sqrt() * ((1 - eta ** 2) ** 0.5)
            

            pred_x0 = pred_x0 - lambda_t*Ap(
                    A(pred_x0.reshape(pred_x0.size(0), -1)) - y.reshape(y.size(0), -1)
                ).reshape(*pred_x0.size())
            # pred_x0 = pred_x0 - lambda_t*Ap(A(pred_x0) - A(x_ori))
    ...
    def step_ddnm(
        ...
        x_prev = alpha_prod_t_prev.sqrt() * pred_x0 + gamma_t * (c1 * torch.randn_like(pred_x0) + c2 * model_output)
        # pred_dir = (1 - alpha_prod_t_prev)**0.5 * model_output
        # x_prev = alpha_prod_t_prev**0.5 * pred_x0 + pred_dir
    ...
    def invert(
        ...
        with tqdm(total=num_inference_steps, desc=desc) as progress_bar:
            for i, t in enumerate(reversed(timesteps)):
                ...
                if ddnm:
                    latents_img = self.next_step_ddnm(noise_pred, t, latents_img,y,A,Ap, ddnm,  strength=strength)
                else:
                    latents_img = self.next_step(noise_pred, t, latents_img)
