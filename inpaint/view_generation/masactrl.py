...
from inpaint.view_generation.masactrl_utils import AttentionBase

from torchvision.utils import save_image


class MutualSelfAttentionControl(AttentionBase):
    def __init__(self, start_step=4, start_layer=10, layer_idx=None, step_idx=None, total_steps=100):
        """
        ...
        self.step_idx = step_idx if step_idx is not None else list(range(start_step, self.total_steps))
...
class MutualSelfAttentionControlCache(AttentionBase):
    def __init__(self, start_step=4, start_layer=10, layer_idx=None, step_idx=None, total_steps=100,strength=0.6):
        """
        Mutual self-attention control for Stable-Diffusion model
        Args:
            start_step: the step to start mutual self-attention control
            ...
            total_steps: the total number of steps
        """
        super().__init__(int(total_steps*strength))
        self.total_steps = total_steps
        self.start_step = start_step
        self.start_layer = start_layer
        self.layer_idx = layer_idx if layer_idx is not None else list(range(start_layer, 16))
    ...
    def forward(self, q, k, v, sim, attn, is_cross, place_in_unet, num_heads, **kwargs):
        ...
        Attention forward function
        """
        if(kwargs.get('hook') and not is_cross):
            self.kv_cache[self.cur_step][self.cur_att_layer]['k'] = k
            self.kv_cache[self.cur_step][self.cur_att_layer]['v'] = v
        
        ## need to add the kwargs.get('hook')
        if is_cross or self.cur_step not in self.step_idx or self.cur_att_layer // 2 not in self.layer_idx:
            return super().forward(q, k, v, sim, attn, is_cross, place_in_unet, num_heads, **kwargs)
   
        if  (len(self.kv_cache.keys()) > 0) \
        and self.kv_cache[self.cur_step][self.cur_att_layer]['k'] is not None \
        and self.kv_cache[self.cur_step][self.cur_att_layer]['v'] is not None:
            k = self.kv_cache[self.cur_step][self.cur_att_layer]['k']
            v = self.kv_cache[self.cur_step][self.cur_att_layer]['v']
        ...
        out_u = self.attn_batch(qu, ku[:num_heads], vu[:num_heads], None,None, is_cross, place_in_unet, num_heads, **kwargs)
        out_c = self.attn_batch(qc, kc[:num_heads], vc[:num_heads], None,None, is_cross, place_in_unet, num_heads, **kwargs)
        # out_u = self.attn_batch(qu, ku[:num_heads], vu[:num_heads], sim[:num_heads], attnu, is_cross, place_in_unet, num_heads, **kwargs)
        # out_c = self.attn_batch(qc, kc[:num_heads], vc[:num_heads], sim[:num_heads], attnc, is_cross, place_in_unet, num_heads, **kwargs)
        out = torch.cat([out_u, out_c], dim=0)
...
class MutualSelfAttentionControlMask(MutualSelfAttentionControl):
    def __init__(self,  start_step=4, start_layer=10, layer_idx=None, step_idx=None, total_steps=100, mask_s=None, mask_t=None, mask_save_dir=None):
        """
