...
def test_cosine():
    prediction = torch.rand(*test_shape)
    loss = F.contextual_loss(prediction, prediction, loss_type='cosine')
    assert loss.shape == torch.Size([])
...
def test_l1():
    prediction = torch.rand(*test_shape)
    loss = F.contextual_loss(prediction, prediction, loss_type='l1')
    assert loss.shape == torch.Size([])
...
def test_l2():
    prediction = torch.rand(*test_shape)
    loss = F.contextual_loss(prediction, prediction, loss_type='l2')
    assert loss.shape == torch.Size([])
