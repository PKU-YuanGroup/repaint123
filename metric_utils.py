...
import argparse
import pandas as pd
# import contextual-loss-pytorch
...
class SSIMMeter:
    ...
    def update(self, preds, truths):
        ...
        for img1, img2 in zip(preds, truths):
            ...
            ssim = compare_ssim(
                img1, img2,
                data_range=1.0, channel_axis=-1)  # assuming your images are scaled to [0,1]
            # print(f"temp psnr {psnr}")
...
#test_dataset = os.listdir(osp(all_inputs, 'test'))
meta_examples = {
   'nerf4': nerf_dataset, 
...
def score_from_method_for_dataset(my_scorer,
                                  ...
                                  pred_path,
                                  score_type='clip', 
                                  rgb_name='render_unet_0', 
                                  result_folder='refine/results', 
                                  first_str=''
                                  ):  # psnr, lpips
    scores = {}
    final_res = 0
    ...
    for i in range(len(examples)):
        ...
        ref_path = osp(input_path, examples[i])
        # if end with rgba.png, continue
        if not ref_path.endswith('_rgba.png'):
            continue
        examples[i] = examples[i].split('_rgba.png')[0]
        # compare entire folder for clip
        if score_type == 'clip' or score_type == 'contextual':
            print(examples[i])
            novel_folder = glob.glob(osp(pred_path,'*'+examples[i]+'*'))
            novel_view = glob.glob(osp(novel_folder[0], result_folder, f'*{rgb_name}*'))
            #novel_view = glob.glob(osp(pred_path,'magic123_'+examples[i]+'*', result_folder, f'*{rgb_name}*'))
            print(f'[INOF] {score_type} loss for example {examples[i]} between 1 GT and {len(novel_view)} predictions')
        # compare first image for other metrics
        else:
            novel_folder = glob.glob(osp(pred_path,'*'+examples[i]+'*'))
            novel_view = glob.glob(osp(novel_folder[0], result_folder, f'{rgb_name}0*'))
            #novel_view = glob.glob(osp(pred_path, 'magic123_'+examples[i]+'*/', result_folder, f'{first_str}{rgb_name}*'))
...
if __name__ == "__main__":
    ...
    contextual_scorer = Contextual(args.device)
    lpips_scorer = LPIPSMeter()
    psnr_scorer = PSNRMeter()
    ssim_scorer = SSIMMeter()
    ...
    for dataset in args.datasets:
        ...
        for pred_path in pred_paths:
            ...
            results_dict = {}
            results_dict['contextual'] = score_from_method_for_dataset(
                contextual_scorer, input_path, pred_path, 'contextual', 
            ...
            results_dict['ssim'] = score_from_method_for_dataset(
                ssim_scorer, input_path, pred_path,  'ssim', 
                result_folder=args.results_folder, rgb_name=args.rgb_name, first_str=args.first_str)
            ...
            print(df)
            df.to_csv(f"{args.save_dir}/{method}-{results_str}-{dataset}-repaint123testmasa.csv")